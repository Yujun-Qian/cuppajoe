...
<p>Moreover you can check a really simple app to teach you how to install shaker: <a class="reference external" href="https://github.com/dferreiroval/mojito-shaker-demos">https://github.com/dferreiroval/mojito-shaker-demos</a></p>
...
<p>Also <tt class="docutils literal"><span class="pre">master</span></tt> will react to various dimensions (skin:grey, device:iphone), so this is how the asset structure looks:</p>
...
dimensions. The <tt class="docutils literal"><span class="pre">master.iphone.css</span></tt> or <tt class="docutils literal"><span class="pre">skin.grey.css</span></tt> files will have some style overrides to change the baseplate style.</p>
</div>
...
<p class="last">By default, every mojit will require two requests, one for the CSS rollup, and another for all the JS. You could improve the number of requests by bundling mojits together. It&#8217;s what we call &#8220;High coverage mojits&#8221;. For more information, read the Usage section.</p>
</div>
</div>
<div class="section" id="runtime">
<h3>Runtime<a class="headerlink" href="#runtime" title="Permalink to this headline">¶</a></h3>
<p>At runtime, the normal workflow happens in mojito until the execution reach the ShakerHTMLFrame. THen our Shaker addon gets executed, looks at the context, determines which dimensions match the request, and serves the most appropriate rollup to the client. So if the context of a request is set to <tt class="docutils literal"><span class="pre">region:CA</span></tt> and <tt class="docutils literal"><span class="pre">device:iphone</span></tt>, Shaker will pick the rollup for those dimensions and attach it to the page.</p>
<div class="admonition note">
<p class="first admonition-title">Note</p>
