    ...
    Y.mojito.controllers[NAME] = {
        ...
        index: function (ac) {
            ...
            var offset = ac.params.merged('offset') || 0,
                limit = ac.params.merged("limit") || ac.config.get('limit'),
                cfg = {
                    children: {
                        feed: {
        ...
        feed: function (ac) {
            ...
            ac.shared_model_addon.load('user').getConfig('user_id', function (error, feedIds) {

                ac.shared_model_addon.load('infinitiefeed').getFeed(feedIds, offset, limit, function (items) {
        ...
        updateTopic: function (ac) {
            ...
            ac.shared_model_addon.load('user').toggleFeed('user_id', topic, function (error) {
                // This could be better by only removing feedIds that were dropped.
                ac.shared_model_addon.load('infinitiefeed').emptyCache();
                ac.done("done");
            });
        },
